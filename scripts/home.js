...
function getBoards() {
    ...
    xhr.open('GET', "http://localhost:8081/api/v1/boards");
...
function addBoardAPI(element) {
    ...
    if (userDetail.role.id == 105) {
        ...
        xhr.open('POST', "http://localhost:8081/api/v1/boards");
        xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
        var access = sessionStorage.getItem('access-token');
        xhr.setRequestHeader('Authorization', "Bearer " + access);
...
function deleteBoard(id) {
    ...
    xhr.open('DELETE', "http://localhost:8081/api/v1/boards/" + id);
    xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
    var access = sessionStorage.getItem('access-token');
    xhr.setRequestHeader('Authorization', "Bearer " + access);
    xhr.send();
...
function addProject(id) {
    ...
    xhr.open('POST', "http://localhost:8081/api/v1/boards/" + id + "/projects");
    xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
    var access = sessionStorage.getItem('access-token');
    xhr.setRequestHeader('Authorization', "Bearer " + access);
    xhr.send(JSON.stringify(params));
...
function showProjects(listID) {
    event.stopPropagation();
    xhr.open('GET', "http://localhost:8081/api/v1/boards/" + listID + "/projects");
    xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
    var access = sessionStorage.getItem('access-token');
    xhr.setRequestHeader('Authorization', "Bearer " + access);
    xhr.send();
